...
if os.path.exists(args.interval) == True:
    ...
    intervals = pd.read_table(intervalFileName, names=[
    ...
    intervals = pd.DataFrame(
        {'chr': [int_chr], 'start': [int_start], 'stop': [int_stop]})
    # save as temporary bedFile
    intervalFileName = str(cwd) + '/' + 'interval.txt'
    intervals.to_csv(intervalFileName, columns=[
...
if vars(args)['outDir'] != None:
    ...
    fileTag = outdir + 'sequencingAlignmentsToMethDensityTable.fileType=' + \
        str(args.fileType) + '.Interval=' + str(intervalTag) + '.OverlapThresh=' + \
            str(overlapTag) + '.readSlice=' + \
                str(args.readSlice) + '.Created=' + str(date)
else:
    fileTag = 'sequencingAlignmentsToMethDensityTable.fileType=' + \
        str(args.fileType) + '.Interval=' + str(intervalTag) + '.OverlapThresh=' + \
            str(overlapTag) + '.readSlice=' + \
                str(args.readSlice) + '.Created=' + str(date)
...
for (dirpath, dirnames, filenames) in walk(path):
    ...
    for f in files:
        ...
        sample_reads = []
        # expects sample same to be the first string before sep in file name.
        sample_name = f.split('.', 1)[0]
        relPath = os.path.relpath(dirpath, path)
        inputFile = os.path.join(path, relPath, f)
